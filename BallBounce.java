      ...
      while(hit==false && i<90){
		...
		if ((BallY> (bricks[i].getYPosition()-25) && 
		   ...
		   (BallX< (bricks[i].getXPosition()+25)))){
			...
			if (temp > 0){							//needs this if statement to ensure that the numbers don't go in to minus numbers
				temp = temp-1;						//minuses 1 to the current number on the brick
				//numbers[i] = new Text ("" + Integer.toString(temp), numPosX,numPosY, 15, "White");		//creates a new number which is -1 to what it was before
				numbers[i].setText("" + Integer.toString(temp));
				//arena.addText (numbers[i]);			//adds the new number onto the bricks
				arena.update();
		
					//bounces the ball off of the side of the brick
